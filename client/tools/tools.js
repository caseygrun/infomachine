...
Workspace.tool.ConnectorHandle = Ext.extend(Workspace.tool.Handle,{
	...
	onDragEnd: function() {
		Workspace.tool.ConnectorHandle.superclass.onDragEnd.apply(this,arguments);
		if(this.dropTarget) {
			switch(this.location) {
				case 'right':
...
Ext.extend(Workspace.tool.PointerTool, Workspace.tool.BaseTool, {
    ...
    */
    ...
    activate: function(){
        ...
        Ext.each(this.workspace.getSelection(),
        function(item){
...
Ext.extend(Workspace.tool.IdeaAdderTool, Workspace.tool.BaseTool, {
    ...
    mouseover: function(e, item){
        if(item && item.wtype=='Workspace.IdeaObject' && item.highlight && Ext.isFunction(item.highlight)) item.highlight();
        // if (item.element){
        //     Ext.fly(item.element).addClass('hover');
        // }
        e.stopEvent();
    ...
    mouseout: function(e, item){
        if(item && item.unhighlight && Ext.isFunction(item.unhighlight)) item.unhighlight();
        // if (item.element){
        //     Ext.fly(item.element).removeClass('hover');
        // }
        e.stopEvent();
